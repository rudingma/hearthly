...
- **node-exporter `node_filesystem_*` is unavailable on these nodes:** the filesystem collector fails with `open /host/proc/1/mountinfo: permission denied` — the confined container cannot read host PID 1's mountinfo under MicroOS's enforcing SELinux (a `CAP_SYS_PTRACE` test did not lift it; the proper fix is an SELinux policy module / privileged context, deferred). The root-disk-fill alerts therefore derive free space from the **btrfs collector** (`node_btrfs_*`), assuming the single-device MicroOS default profile (Data=single, Metadata/System=DUP). Verified ±1% against `df`. If a node ever uses a non-btrfs root or multi-device btrfs, revisit the formula.
- **Monitoring egress (default-deny):** Prometheus needs API-server egress for service discovery AND node-subnet egress (`10.0.0.0/16` + `10.255.0.0/16` on 9100/10250) for node-exporter/kubelet; kube-state-metrics needs its own API-server egress or it crash-loops. All in `network-policies/templates/monitoring.yaml`. A missing API egress silently blinds Prometheus (0 targets) — the symptom that hid the 2026-05 outage.
