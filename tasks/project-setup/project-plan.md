...
| 7: Terraform k3s cluster | Done | kube-hetzner v2.18.5, k3s v1.34.5→v1.34.6 (auto-upgraded), 1 CP + 3 workers (CAX11 ARM) in nbg1. Traefik LB: 46.225.42.23. Fixed: Traefik chart v34+ schema, CRLF→LF on WSL2, fsn1→nbg1. MicroOS snapshots via Packer. Remote state on Hetzner Object Storage (S3 backend). Traefik HelmChart recreated with correct values (CrashLoopBackOff fix). |
| 9: DNS | Done | Cloudflare DNS (not Hetzner — Cloudflare Registrar locks NS). A records for @, api, argocd, grafana, secrets → 46.225.42.23. DNS only (grey cloud, no proxy). LB IP changed once during Traefik HelmChart recreation. |
...
| 11: ArgoCD bootstrap | Done | ArgoCD v3.3.6 (chart v9.4.17). Self-managing via multi-source Application. Root app-of-apps pattern: new apps auto-deployed by dropping YAML in applications/ directory. HTTPS repo access (SSH blocked by Hetzner firewall outbound). Deploy key on GitHub, token in K8s Secret. Dex + notifications disabled. Review fixes applied: ingressClassName on all Ingresses, ArgoCD TLS cert, root app-of-apps, safer sed in CI, npm audit continue-on-error. |
...
| 15: Monitoring | Done | kube-prometheus-stack v82.16.0 (Prometheus v3.10.0, Grafana 12.4.2) in monitoring namespace. Grafana at grafana.hearthly.dev with Let's Encrypt TLS. Prometheus: 10d retention, 10Gi storage, 30s scrape, 23/23 targets up. Node exporter DaemonSet (4 pods). kube-state-metrics for K8s object metrics. k3s-specific: disabled kubeEtcd/kubeControllerManager/kubeScheduler/kubeProxy (bind to localhost). Alertmanager disabled for Phase 1. Workarounds: (1) admission webhook Helm hooks blocked ArgoCD sync — disabled webhooks (only validate PrometheusRule CRDs, not needed Phase 1); (2) ArgoCD controller OOM'd at 1Gi processing ~80 CRDs — increased to 2Gi. Infra monitoring only; app observability (OTel) deferred to Phase 2. |
| 15 (dashboard) | Done | Custom "Hearthly Cluster Health" dashboard designed from SRE first principles (USE method). 22 panels: emergency signals (OOMKills, CrashLoop, nodes ready, failed backups), per-node CPU/memory gauges, saturation (CPU throttling, memory-vs-limit), disk (rootfs + PVCs), resource trends, namespace breakdown, problem pods, Prometheus self-health. Disabled 23 generic bundled dashboards. Grafana API service account for Claude Code access. |
| 16: Backups | Done | Daily pg_dump CronJob (02:00 UTC) with custom format, SHA256 checksums. Two-container pattern: postgres:18-alpine (init, dump) + amazon/aws-cli (upload). Hetzner Object Storage bucket `hearthly-backups` with S3 lifecycle policy (30-day auto-expiry). S3 credentials managed in Infisical. Hardened: pipefail for pipe failure detection, file size validation, readOnlyRootFilesystem. Evaluated CNPG native backups — not viable on Hetzner (S3 restore bug, plugin pre-1.0). Backup monitoring panels in Grafana dashboard. Restore docs in CLAUDE.md. |
| 17: End-to-End | Done | All 6 ArgoCD apps Healthy. All 5 HTTPS endpoints responding with valid Let's Encrypt TLS (R12/R13). 23/23 Prometheus targets up. Infisical syncing 2 secrets. Backup CronJob scheduled. 4/4 nodes Ready. README.md written. |
